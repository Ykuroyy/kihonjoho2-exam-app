...
このグラフは辺の配列 {{1, 3}, {1, 4}, {3, 4}, {2, 4}, {4, 5}} で表現される。
...
グラフの頂点には，1 から順に整数で番号が付けられている。グラフは無向グラフであり，各頂点間には高々一つの辺がある。一つの辺は両端の頂点の番号を要素にもつ要素数 2 の整数型の配列で表現できる。例えば，{1，3} は頂点 1 と頂点 3 を端点とする辺を表す。
...
例：商品AとBの関連度LAB = (2 × 6) ÷ (4 × 3) = 1.0
（MAB=2（注文1,4）、全注文数=6、KA=4（注文1,2,3,4）、KB=3（注文1,4,5））
